...
function calculateAverages(data, callback) {
  ...
  console.log("-- RAPTOR MINER: ", data.miner, " --");
  console.log(
    "---------------------------------------------------------------------"
  );
...
axios
  .get("http://vdl.raptorpool.org/api/stats")
  .then((response) => {
...
function run(minersAddress) {
  ...
  axios
    .get("http://vdl.raptorpool.org/api/worker_stats?" + minersAddress)
    .then((response) => {
      ...
      setTimeout(function () {
        calculateAverages(response.data, function (
          ...
          axios
            .get("http://vdl.raptorpool.org/api/payments")
            .then((response) => {
              // console.log(response.data[0].pools.vidulum);
              determinePaidOut(
                response.data,
